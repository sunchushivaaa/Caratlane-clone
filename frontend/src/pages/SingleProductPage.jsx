...
const CartPage = () => {
    ...
    useEffect(() => {
        ...
        if(ringdata && ringdata.Data && ringdata.Data.length==1){
         ...
         else if(braceletdata && braceletdata.Data && braceletdata.Data.length===1){
            console.log(braceletdata)
          setProd(braceletdata.Data[0])
         }
         else if(earringsdata && earringsdata.Data && earringsdata.Data.length===1){
            console.log(earringsdata)
          setProd(earringsdata.Data[0])
         }
         else if(mangalsutradata && mangalsutradata.Data && mangalsutradata.Data.length===1){
            console.log(mangalsutradata)
    ...
    return (
        <Box width="100%">
            ...
            <Box display={["block", "block", "flex", "flex"]}width="100%" marginTop="1%">
                <Box width={["100%", "100%", "50%", "50%"]}  height={["250px","250px","500px","500px"]} padding="4%">
                   <Image src={prod.img1} width="100%" height="100%"/>
                </Box>
                <Box width={["100%", "100%", "40%", "40%"]} textAlign="left" padding="2%" >
                    <Box width="100%">
                        <Heading as="h2" color="violet">{prod.name}</Heading>
                        <Box width="100%" height="300px" marginTop="2%" padding="3%" boxShadow="rgba(0, 0, 0, 0.05) 0px 0px 0px 1px, rgb(209, 213, 219) 0px 0px 0px 1px inset">
                            <Box marginTop="4%" className='order_summary_box'><p>MRP :     {prod.MRP}</p></Box>
                            <Box marginTop="4%"  className='order_summary_box'><p>Details :     {prod.details} </p></Box>
                            <Box marginTop="4%"  className='order_summary_box'><p>Purity :     {prod.purity} </p></Box>
                            <Box marginTop="4%"  className='order_summary_box'><p>Weight:      {prod.weight} </p></Box>
                        </Box>
