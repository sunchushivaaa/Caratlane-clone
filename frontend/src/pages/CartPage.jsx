  ...
  return (
    <Box width="100%">
        ...
        <Box display={["block","block","flex","flex"]}  width="100%" marginTop="1%">
             <Box width={["100%","100%","65%","65%"]} overflow="scroll">
             { data && data.length>0 && data.map(ele=><Singleproduct key={ele.id} {...ele}/>)}
             </Box>
             <Box width={["100%","100%","30%","30%"]} position={["initial","initial","fixed","fixed"]} left="70%" >
              <Box width="80%" margin="auto">
                <h6>Apply Coupon: <b>"The Originals" for 10% discount</b></h6>
                <HStack justifyContent="space-between" width="100%" >
                <Input type="text" value={coupon} onChange={(e)=>setCoupon(e.target.value)} width="80%"/>
                 <Button onClick={()=>handleSubmit()} width="20%">Apply</Button>
                 </HStack>
              </Box>
                  <Box width="80%" margin="auto" marginTop="4%">
                     ...
                     <Box width="100%" boxShadow="rgba(0, 0, 0, 0.05) 0px 0px 0px 1px, rgb(209, 213, 219) 0px 0px 0px 1px inset">
                        <Box className='order_summary_box'><p>Subtotal :            {price+data.MRP} </p></Box> 
